;( function (w, ko){
	...
	var x = w.RMS.XHR, _base = w.RMS.baseUrl,
		...
		}, me = familyBackgroundVM, url = w.location.pathname, warning,
		lastSlash = url.lastIndexOf("/"),urlLen = url.length,
		targetID = parseInt( url.substr( lastSlash + 1, urlLen + 1) ), hasValidID = false;
		...
		me.saveRecord = function(){
			...
			x.post( _base + "family-background/store", {data}).done(function(res){
				if( res ){
					new PNotify({
						title: 'Success!',
	...
	me.updateRecord = function(){
		...
		x.post( _base + "family-background/update/" + me.updateID(),{data}).done( function ( response ){
			if( response > 0){
				new PNotify({
					title: 'Success!',
					text: 'Updated!',
