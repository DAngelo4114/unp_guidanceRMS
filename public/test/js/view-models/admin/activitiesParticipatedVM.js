;( function (w, ko){
	...
	}, me = activitiesParticipatedVM, success, error;
	...
	me.student_id.subscribe( function() {
		if( me.student_id() ){
			if(me.activityFields().length < 1){
				me.activityFields([new activityField(me.student_id().id)]);
			}else{
	...
	me.addField = function(){
		me.activityFields.push( new activityField(me.student_id().id) );
	};
	...
	me.updateRecord = function(){
		var data = ko.toJS(me.activityFields())
		x.post( _base + "activity/update",{data}).done( function ( response ){
			if( response == "OK"){
				new PNotify({
					title: 'Success!',
					text: 'Updated!',
