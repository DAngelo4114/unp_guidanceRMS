;( function ( w, d, $, ko ){
	...
	x.get( _base + "ajax-source/statistics").done( function ( response ){
		var genderPerCollegesData = response.genderPerColleges, 
			genderWholeData = response.genderWhole[0], byStatus = response.byStatus[0];
	...
	var genderPerCollegeChart = new Chart(genderPerCollegeCTX).Bar(new __barChart(genderPerCollegesData)),
		...
		byStatusChart = new Chart(byStatusCTX).Pie( new pie2(['Single','Married','Widow/Widower'],
			[byStatus.single, byStatus.married, byStatus.widow]));
