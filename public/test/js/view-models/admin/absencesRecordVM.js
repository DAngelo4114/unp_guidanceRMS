;( function (w, ko){
	...
	}, me = absencesRecordVM, error, success;
	...
	me.student_id.subscribe( function() {
		if( me.student_id() ){
			if(me.absencesRecordFields().length < 1){
				me.absencesRecordFields([new absencesRecordField(me.student_id().id)]);
			}else{
	...
	me.addField = function(){
		me.absencesRecordFields.push( new absencesRecordField(me.student_id().id) );
	};
	...
	me.updateRecord = function(){
		var data = ko.toJS(me.absencesRecordFields())
		x.post( _base + "absent/update",{data}).done( function ( response ){
			if( response == "OK"){
				new PNotify({
					title: 'Success!',
					text: 'Updated!',
